...
# --- 2. 免責聲明 (置頂渲染) ---
st.markdown("""
<div style="background:#fff3e0; padding:15px; border-radius:10px; border:2px solid #ff9800; margin-bottom:25px; font-family: sans-serif;">
    <h3 style="color:#ef6c00; margin-top:0;">⚠️ 系統使用免責聲明 (Disclaimers)</h3>
    <p style="color:#5d4037; font-size:0.9em; line-height:1.6; margin-bottom:0;">
        本平台「半導體大戶戰情室」所提供之所有數據、AI 診斷分析、技術指標及成本模擬結果，<b>僅供學術研究與程式開發測試參考，不構成任何形式的投資建議、邀約或推薦。</b><br>
        金融市場瞬息萬變，AI 生成內容可能存在時滯或誤差。使用者在進行任何金融決策前，應諮詢專業顧問並自行評估風險。本系統開發者對使用者之投資損益不負任何法律責任。
    </p>
</div>
    ...
    <div style="background:#fff9f9; border-left:12px solid #e53935; padding:20px; border-radius:12px; border:1px solid #ffdde0; font-family: sans-serif; margin-bottom: 30px;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:10px;">
            <div>
                ...
                <div style="font-size:3.5em; font-weight:900; color:#b71c1c; margin:5px 0;">${p}</div>
                <div style="font-size:0.9em; color:#555;">趨勢：💀 空頭排列 | <b style="color:#1a237e;">PE: {pe}</b> | <b style="color:#1a237e;">成長: {gr}</b></div>
            </div>
        ...
        <div style="background:white; border:1px solid #eee; border-radius:10px; padding:15px; margin-bottom:15px;">
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap:15px; text-align:center;">
                ...
                <div><span style="color:#c62828; font-size:0.8em; font-weight:bold;">🎯 壓力位</span><br><b style="color:#c62828; font-size:1.4em;">{pre}</b></div>
                <div><span style="color:#c62828; font-size:0.8em; font-weight:bold;">📉 支撐分佈</span><br><b style="color:#c62828; font-size:1.4em;">{sup}</b></div>
                <div><span style="color:#c62828; font-size:0.85em; font-weight:bold;">📈 壓力分佈</span><br><b style="color:#c62828; font-size:1.4em;">{pre}</b></div>
            </div>
            <div style="margin-top:15px; padding-top:10px; border-top:1px solid #f0f0f0; display:flex; flex-wrap:wrap; gap:15px;">
                <div style="flex:1; min-width:200px;">
                    ...
                    <span style="color:#1565c0; font-size:0.85em;">防守觀察點: {sup}</span> | <span style="color:#c62828; font-size:0.85em;">ATR地板: 68.41</span>
                </div>
        ...
        <div style="background:rgba(255,255,255,0.8); padding:15px; border-radius:10px;">
            ...
            <div style="margin-top:10px; font-size:1em; line-height:1.6; color:#444;">{report_html}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
