...
# 1. 頁面配置
st.set_page_config(page_title="半導體大戶戰情室-全數據版", layout="wide")

# 2. 注入自定義 CSS
st.markdown("""
    <style>
    .status-card { padding: 25px; border-radius: 15px; margin-bottom: 20px; border: 1px solid #e0e0e0; box-shadow: 2px 2px 10px rgba(0,0,0,0.1); position: relative; }
    ...
    .metric-tag { display: inline-block; padding: 4px 10px; background: rgba(0,0,0,0.06); border-radius: 6px; margin-right: 8px; font-size: 0.85em; font-weight: bold; color: #333; }
    .price-main { font-size: 2em; font-family: monospace; font-weight: bold; color: #1a1a1a; margin: 10px 0; }
    ...
    .price-buy { color: #1b5e20; font-size: 1.5em; font-weight: 900; }
    .price-sell { color: #c62828; font-size: 1.5em; font-weight: 900; border-top: 1px dashed #ccc; margin-top: 8px; padding-top: 8px; }
    
    /* 倒數計時器樣式 */
    .timer-text { color: #ff5722; font-weight: bold; font-size: 1.2em; text-align: right; margin-bottom: 10px; }
    </style>
    ...
    for ticker, name in tickers.items():
        try:
            ...
            if vol_ratio > 2.2 and abs(slope) < 0.1:
                icon, style, status = "🚨", "🚨", "【大戶出貨】爆量不漲。籌碼換手跡象明顯，風險極高，建議減碼。"
            elif close >= tech_press * 0.98:
                icon, style, status = "⚠️", "⚠️", f"【分批停利】已接近目標壓力位 {tech_press:.2f}。短線過熱，建議獲利落袋。"
            elif close <= buy_price * 1.03 and slope > -0.1:
                icon, style, status = "✅", "✅", f"【黃金買點】回測籌碼地板 {chip_floor:.2f}。支撐穩健，適合分批佈局。"
            elif close <= tech_sup:
                icon, style, status = "☢️", "☢️", "【破位重挫】擊穿關鍵下軌。慣性已破，請嚴格執行停損。"
            else:
            ...
            data_list.append({
                ...
                "insider": f"{insider_pct:.1f}%", "sup": f"{tech_sup:.2f}", 
                "floor": f"{chip_floor:.2f}", "bias": f"{bias:.1f}%"
            })
    ...
    <div class="status-card {d['style']}">
        <div style="display: flex; justify-content: space-between;">
            <div style="flex: 2;">
                ...
                <div>
                    <span class="metric-tag">預估PE: {d['pe']}</span>
            ...
            <div class="target-box">
                <div style="font-size:0.85em; color:#555; font-weight:bold;">💡 建議買入價</div>
                <div class="price-buy">{d['buy']}</div>
                <div style="font-size:0.85em; color:#555; font-weight:bold; margin-top:8px;">🎯 建議停利價</div>
                <div class="price-sell">{d['sell']}</div>
            </div>
        </div>
...
for i in range(60, 0, -1):
    timer_placeholder.markdown(f"<div class='timer-text'>🔄 {i} 秒後自動刷新</div>", unsafe_allow_html=True)
    time.sleep(1)
