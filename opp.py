...
# 1. 頁面配置
st.set_page_config(page_title="半導體大戶戰情室-動態加權版", layout="wide")
...
st.markdown("""
    ...
    .defense-box { background: rgba(255, 255, 255, 0.6); border: 1px dashed #8c8c8c; padding: 10px; border-radius: 8px; margin-top: 10px; }
...
with col_t: st.title("🖥️ 半導體大戶戰情室 (方案B：動態加權)")
with col_r: timer_placeholder = st.empty()

data_list = []
news_dict = {}

with st.spinner('正在分析趨勢並計算加權買點...'):
    for ticker, name in tickers.items():
        try:
            ...
            if slope_pct > 0:
                # 多頭：70% 技術支撐 + 30% 籌碼地板 (買高一點避免踏空)
                suggested_buy = (tech_support * 0.7) + (chip_floor * 0.3)
            else:
                # 空頭：30% 技術支撐 + 70% 籌碼地板 (買低一點防止接刀)
            ...
            if vol_ratio > 2.0 and abs(slope_pct) < 0.1 and close_val > ma20:
                ...
                icon, style, status = "☢️", "☢️", f"☢️ 【停損警示】股價已擊穿技術支撐 {tech_support:.2f}。趨勢慣性已被破壞，請嚴格執行停損。"
            else:
            ...
            data_list.append({
                ...
                "diag": status, "inst": f"{inst_pct:.1f}%",
        ...
        <div style="display: grid; grid-template-columns: 2fr 1.2fr; gap: 20px;">
            <div>
                ...
                <div class="defense-box">
                    🛡️ <b>防禦體系：</b> 
                    <span style="color:#1890ff;">停利防線: {d['stop_line']}</span> | 
