...
def check_password():
    if st.session_state.get("password_correct", False):
        return True
    st.markdown("###  prenatal 內部開發監測系統 V6.8")
...
def get_ai_analysis(name, price, rsi, chip_flow, trend, pe, rev, bias):
    ...
    prompt = f"""
    標的: {name}, 現價: {price}, RSI: {rsi:.1f}, 籌碼: {chip_flow}, 趨勢: {trend}, 本益比: {pe}, 營收成長: {rev}, 乖離率: {bias}%。
    你是資深首席分析師，請針對以上量價與基本面數據，給出『極簡一句話』的部署建議。
    禁止任何廢話與開場白，直接給核心策略，限 50 字內。
    """
    try:
    ...
    for ticker, info in tickers.items():
        try:
            ...
            ai_score, ai_diag, ai_style = calculate_ai_confidence(
                {'trend': trend_label, 'chip_flow': chip_flow, 'price': close_val, 'rsi': rsi_val, 'pe': pe_val, 'rev': rev_growth, 'bias': bias},
            ...
            data_list.append({
                ...
                "ai_diag": ai_diag, "buy": round(suggested_buy, 2), "sell": round(tech_pre, 2), "pe": pe_val, "rev": rev_growth,
                ...
                "bias": bias, "sup": round(tech_sup, 2), "pre": round(tech_pre, 2),
                "inst": inst_hold, "chip_flow": chip_flow, "trend": trend_label
            })
        except: continue
...
for d in data_list:
    ...
    <div class="status-card {d['style']}">
        ...
        <div style="display: flex; gap: 25px;">
            <div style="flex: 2.2;">
                <b>🧠 智權診斷 (Groq 極簡部署)：</b><br><span style="line-height:1.6; font-size:1.1em;">{d['ai_diag']}</span>
                <div class="defense-box">
                    ⚙️ <b>風控與成本模擬：</b> 
                    <span style="color:#1890ff;">營利防守: {d['stop_line']}</span> | 
                    <span style="color:#cf1322; font-weight:bold;">ATR地板: {d['stop']}</span> <br>
                    <b>換手支撐區: {d['chip_floor']}</b> | 統計支撐: {d['sup']} | 斜率: {d['slope']}%
                </div>
            </div>
