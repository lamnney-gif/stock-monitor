...
for ticker, name in tickers.items():
    ...
    # 處理 AI 文本：將換行轉換為 HTML 標籤，並處理特殊字元防止 markdown 誤判
    report_raw = ai_db.get("reports", {}).get(ticker, "🤖 分析同步中...")
    report_cleaned = report_raw.replace("\n", "<br>")
    ...
    # 使用不含大括號衝突的 HTML 結構
    # 移除 height 與 overflow，讓容器自動延伸
    full_card = f"""
    <div style="background:#fff9f9; border-left:12px solid #e53935; padding:20px; border-radius:12px; border:1px solid #ffdde0; font-family: sans-serif; margin-bottom: 25px; width: 100%; box-sizing: border-box;">
        ...
        <div style="background:white; border:1px solid #eee; border-radius:10px; padding:15px; margin-bottom:15px;">
            <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap: wrap; gap:15px;">
                ...
                <div style="flex:1.5; min-width:180px; padding-left:10px; border-left: 2px solid #f0f0f0;">
                    ...
                    <span style="color:#1565c0; font-size:0.85em;">防守點: {sup}</span> | 
        ...
        <div style="background:rgba(255,255,255,0.8); padding:15px; border-radius:10px;">
            ...
            <div style="margin-top:10px; font-size:1em; line-height:1.6; color:#444;">
                {report_cleaned}
            </div>
        </div>
    </div>
    """
    # 使用 unsafe_allow_html 渲染，但包裹在一個 div 避免 markdown 誤判
    st.write(full_card, unsafe_allow_html=True)
